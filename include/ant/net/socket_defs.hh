...
public:
    ...
    socket_address(const net::inet_address &, uint16_t);
...
struct ipv6_addr {
    ...
    ipv6_addr(const net::inet_address &, uint16_t p);
...
template<>
struct hash<ant::socket_address> {
    size_t operator()(const ant::socket_address &) const;
};

template<>
struct hash<ant::ipv4_addr> {
    size_t operator()(const ant::ipv4_addr &) const;
};
